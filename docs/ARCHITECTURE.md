...
This document describes the implementation contracts behind KikiFast. For
installation and operator-facing configuration, start with the project
[README](../README.md). The phased senior-care checklist lives in
[SENIOR_CARE_ROADMAP.md](SENIOR_CARE_ROADMAP.md).

KikiFast is a low-latency voice-companion runtime designed for a Raspberry Pi 5.
It coordinates wake-word detection, speech recognition, local or cloud language
models, speech synthesis, vision, robot controls, memory, scheduled workers, and
senior-care routines. Hardware-facing integrations are optional and fail closed
when their services are unavailable.

Runtime data belongs under `runtime/` and is intentionally excluded from Git.
Secrets belong in `.env` or an external credential store. Checked-in defaults
are provided by `tools_and_config/config.example.json`.

## 1. Service topology

| Component | Default endpoint | Configuration |
|---|---:|---|
| KikiFast orchestrator | local process | `main.py` |
| llama.cpp-compatible LLM | `http://127.0.0.1:8080/v1` | `llm.local_api_base` |
| TTS server | `http://127.0.0.1:8082` | `tts.local_url` |
| whisper.cpp server | `http://127.0.0.1:5555/inference` | `stt.whisper_url` |
| Camera snapshot service | `http://127.0.0.1:5000` | `llm.instant_vision.*_url` |
| Hailo/controller bridge | `127.0.0.1:5555/5556` | `controller.*` |
| Web dashboard | `http://127.0.0.1:8090` | `webui/server.py` |
| Optional WhatsApp bridge | `http://127.0.0.1:8080/api` | `whatsapp.*` |

The local llama.cpp server is treated as a single high-priority slot. Foreground
speech may preempt background generation, and background completion rewarms the
speaking prefix. Section 4 documents the cache contract.
...
├── kiki_boot.py                    # Raspberry Pi boot orchestration
├── assets/models/                  # Checked-in wake-word model
├── core/                           # Runtime services and domain logic
│   ├── brain/                      # Memory, reasoning, and background cognition
│   ├── hardware/                   # ZMQ controller and MAX30102 adapters
│   ├── health/                     # Environment and care snapshots
│   ├── self_extend/                # Optional MCP and skill integrations
│   ├── senior/                     # Senior-care plans and sessions
│   ├── vision/                     # Camera and visual-query routing
│   └── workers/                    # Background worker scheduler
├── docs/                           # Architecture and feature documentation
├── hotwords/                       # Wake-word recognizer
├── integrations/webrtc/            # Optional browser audio bridge
├── plugins/                        # Built-in behavior plugins
├── robot/                          # Face, neck, and OLED adapters
├── scripts/                        # Diagnostics, demos, and operational scripts
├── skills/                         # Runtime skill definitions
├── sound_effects/                  # Application audio assets
├── tests/                          # Unit and opt-in integration tests
├── tools_and_config/               # Configuration, logging, and tool dispatch
└── webui/                          # Local dashboard
```

`whisper.cpp` and the WhatsApp MCP implementation are external optional
dependencies. Configure their locations with the environment variables shown in
`.env.example`; no third-party source, session database, or downloaded toolchain
...
connects the paired `bluetooth_speaker` (the configured Bluetooth speaker; configured MAC first with paired-name
...
because a speaker can broadcast a name the config never learned (`the configured Bluetooth speaker` in
`config.json` vs `the discovered Bluetooth speaker` over the air). A loose match is logged.
...
supports idle mobile hotspots and configured hidden SSIDs; their password connection
...
the external `whisper-cli` and model configured through environment variables. Dictation accepts forms
...
Verified end-to-end by `scripts/benchmarks/llama_server/test_prefill_e2e.py` (cold 24s → warm 1.26s;
...
Optimized pipeline ported from the standalone `scripts/diagnostics/whisper_latency.py` benchmark: per-frame
...
OpenWakeWord on `assets/models/kiki.onnx`, threshold 0.5, 4.5s detection cooldown. The key mechanism is
...
- **`logger.py` (111)** — `_Tee` wraps stdout/stderr: every write mirrored to
  `runtime/logs/kikifast.log` with a per-line `HH:MM:SS.mmm` prefix; size rotation at `max_bytes`
...
- **`tools.py` (1678)** — all tools. Layout:
  - utilities @25–170: `should_skip_followup` (one-shot flag set by play_music/dance so
    ...
    (credential read from `EXA_API_KEY`), shared KikiController getter.
  - tool impls @174–1052 (all async): `search_web` (Exa, 3 results, highlights, IN locale,
    ...
    (subprocess with the `python` venv), five compact
...
`source .venv/bin/activate && python tests/calibrate_lcd_sync.py`.
...
### 5.20 `scripts/benchmarks/llama_server/test_prefill_e2e.py`
...
### 5.21 `core/hardware/controller.py` (≈380 lines)
...
face_lost / training_complete) against the Hailo pipeline at `controller.host` (127.0.0.1).
...
- **MAX30102 heart rate:** root `core/hardware/max30102.py` retains its CLI and also exposes structured
...
- **Injected on CHANGE, never on a timer.** `CareNowInjector` appends only when the rendered line
  ...
  band edge cannot inject every turn. This is the deterministic layer docs/SENIOR_CARE_ROADMAP.md asks for under the
  AI's wording.
...
paths**. Per the architecture decision in `docs/SENIOR_CARE_ROADMAP.md`, a care event stores a rich goal/context
...
12. **Secrets**: API keys are read from environment variables, and Vertex AI uses the
    external file named by `GOOGLE_APPLICATION_CREDENTIALS`. Runtime credentials and
    service session databases must never be committed.
...
15b. **yt-dlp rots, and it fails as silence, not as an error.** A stale build resolves the
    ...
    (`python -m pip install -U yt-dlp`), 206 means the problem is the sink.
